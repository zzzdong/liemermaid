...
本目录收录 **Mermaid 官方语法定义** 的完整参考，供 liemermaid（基于 Pest 的 Mermaid 解析器/渲染器）实现与校验时对照使用。

资料来源：Mermaid 官方文档 <https://mermaid.js.org/syntax/>，覆盖至 v11.x。

## 图表类型与文档对照

本项目 `src/grammar/mermaid.pest` 当前支持以下 8 种图表类型，各语法文档如下：
...
- **注释**：以 `%%` 开头，直到行尾；解析器忽略。
- **空白**：空格、Tab、换行均为空白。
- **标识符**：字母开头，可含字母、数字、下划线、连字符；支持 Unicode（需用双引号包裹时按各图规则）。

## 注意事项

- 官方文档更新较快，本文档标注了各语法特性引入的最低版本（如 `v11.16.0+`）。
- 部分语法为实验性功能（如 timeline），后续版本可能变更。
- 若与 `src/grammar/mermaid.pest` 现有实现不一致，以官方文档为准，再决定是否扩展 pest 规则。

## 相关文档

- [parser.md](parser.md) — 基于 Pest 的 Mermaid 解析器架构设计
